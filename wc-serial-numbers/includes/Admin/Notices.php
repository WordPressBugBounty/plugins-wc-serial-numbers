...
class Notices {
	...
	public function add_notices() {
		$pro_version = '';
		foreach ( array( 'WCSN_PRO_VERSION', 'WC_SERIAL_NUMBER_PRO_PLUGIN_VERSION' ) as $const ) {
			if ( defined( $const ) ) {
				$pro_version = constant( $const );
				break;
			}
		}
		$is_outdated_pro = '' !== $pro_version && version_compare( $pro_version, '1.4.0', '<' );
		if ( ! $is_outdated_pro ) {
